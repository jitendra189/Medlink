...
describe('BloodService', () => {
  ...
  const mockDonor: Partial<BloodDonorEntity> = {
    id: 'd-1', userId: 'u-1', bloodGroup: BloodGroup.O_POS, isAvailable: true, totalDonations: 0,
  };
  const mockRequest: Partial<BloodRequestEntity> = {
    id: 'r-1', patientId: 'p-1', bloodGroup: BloodGroup.O_POS, status: BloodRequestStatus.PENDING,
  };
      ...
      providers: [
        ...
        },
        {
          ...
          useValue: {
            ...
            findOneOrFail: jest.fn().mockResolvedValue(mockRequest),
  ...
  it('cancels only a request owned by the patient', async () => {
    ...
    expect(requestRepo.update).toHaveBeenCalledWith(
      { id: 'r-1', patientId: 'p-1', status: BloodRequestStatus.PENDING },
